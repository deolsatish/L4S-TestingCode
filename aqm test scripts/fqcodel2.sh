...
ssh -p $router1port -i ~/.ssh/$sshkey root@$vmhostaddr "ipfw sched 1 config pipe 1 type fq_codel"
...
ssh -p $router2port -i ~/.ssh/$sshkey root@$vmhostaddr "ipfw sched 1 config pipe 1 type fq_codel"
